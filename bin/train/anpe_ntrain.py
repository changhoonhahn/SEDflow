...
i_train = np.random.choice(np.arange(x_train.shape[0]), size=ntrain, replace=False)
...
fsumm = os.path.join(Train.data_dir(), 'anpe_thetaunt_magsigz.%s.ntrain%i.%ix%i.%i.p' % (sample, ntrain, nhidden, nblocks, itrain))
