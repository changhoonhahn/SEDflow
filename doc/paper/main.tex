...
\begin{abstract}
    ...
    In this work, we present an alternative \emph{scalable} method for
    ...
    Once trained, deriving the posterior of galaxy properties takes \emph{${\sim}1$
    second per galaxy}. 
    To demonstrate our method, we combine ANPE with the recent \cite{hahn2022}
    ...
    We then validate our method using posteriors derived from standard Markov
    Chain Monte Carlo (MCMC) sampling and mock observations, with known true
    parameter values.  
    The posteriors from ANPE accurately estimate the true posteriors of SED
    ...
    We apply our method to 33,887 galaxies in the NASA-Sloan Atlas and publicly
    release the posteriors of galaxy properties for every galaxy.
\end{abstract}
