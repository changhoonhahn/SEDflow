...
\begin{abstract}
    ...
    (\eg~DESI, PFS, Rubin, Webb, and Roman). 
    ...
    We present, and publicly release, \sedflow, an ANPE method to produce posteriors 
    of the recent \cite{hahn2022} SED model from optical photometry.
    With \sedflow~it takes \emph{${\sim}1$ second per galaxy} to obtain the posterior
    distributions of 12 model parameters, all of which are in excellent agreement with
    traditional Markov Chain Monte Carlo sampling results.
